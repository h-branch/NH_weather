...
df_regions = pd.DataFrame({"지역명": txt})
df_regions["지역명"] = (
    df_regions["지역명"]
    .astype(str)
    .str.strip()
...
df_regions = df_regions[df_regions["지역명"] != ""].copy()
...
sido_code_map = {
    ...
    "경북": "47",
    "경남": "48",
    # "제주": "50"  # 주의: 여기서는 제주를 제주시로 보기 위해 시도명에서 제외
}
# 시도명 여부
df_regions["is_sido"] = df_regions["지역명"].isin(sido_code_map.keys())
# 시도명만 추출
df_sido = df_regions[df_regions["is_sido"]].copy()
# 시도별 언급 횟수
sido_count_table = (
    df_sido["지역명"]
    .value_counts()
...
sido_count_table.columns = ["시도명", "sido_count"]
sido_count_table["code_prefix"] = sido_count_table["시도명"].map(sido_code_map)
display(sido_count_table)

# gpd_sgg 복사 및 CODE 문자형 변환
gpd_sgg_count = gpd_sgg.copy()
gpd_sgg_count["CODE"] = gpd_sgg_count["CODE"].astype(str)
# 시군구명만 추출
df_sgg = df_regions[~df_regions["is_sido"]].copy()
# 원문 앞 2글자 key
df_sgg["sgg_key"] = df_sgg["지역명"].str[:2]
# gpd_sgg 쪽도 앞 2글자 key
gpd_sgg_match = gpd_sgg_count[["CODE", "SGG", "SD_CD"]].copy()
gpd_sgg_match["sgg_key"] = gpd_sgg_match["SGG"].str[:2]
# 앞 2글자 기준 매칭
df_sgg_matched = df_sgg.merge(
    gpd_sgg_match[["CODE", "SGG", "SD_CD", "sgg_key"]],
    on="sgg_key",
    how="left"
)
display(df_sgg_matched[["지역명", "sgg_key", "CODE", "SGG", "SD_CD"]].head(30))
...
count_table_sgg = (
    ...
    .size()
    .rename(columns={"size": "sgg_count"})
    .sort_values("sgg_count", ascending=False)
)
display(count_table_sgg.head(30))
...
gpd_sgg_count["sgg_count"] = gpd_sgg_count["sgg_count"].fillna(0).astype(int)

gpd_sgg_count["code_prefix"] = gpd_sgg_count["CODE"].str[:2]
gpd_sgg_count = gpd_sgg_count.merge(
    sido_count_table[["code_prefix", "sido_count"]],
    on="code_prefix",
    how="left"
)
gpd_sgg_count["sido_count"] = gpd_sgg_count["sido_count"].fillna(0).astype(int)

gpd_sgg_count["언급횟수"] = (
    gpd_sgg_count["sgg_count"] + gpd_sgg_count["sido_count"]
)
display(
    gpd_sgg_count[["CODE", "SGG", "sgg_count", "sido_count", "언급횟수"]]
